...
Figure~\ref{fig:sysarch} shows the end-to-end pipeline: TPTP input is
parsed and built into an initial sequent, then dispatched to one of the
three engines. Each engine differs only in which scheduler policy is
injected and whether an iterative-deepening envelope wraps the shared
\texttt{backwards\_search} kernel.
...
\begin{figure}[t]
  ...
  \caption{System architecture of the theorem prover. Engine selection
    determines both search strategy and the injected scheduler;
