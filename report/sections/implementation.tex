...
engine, and a CLI front-end. The parser uses
\texttt{pest}~\cite{pest2024}, a parsing-expression-grammar library,
against a hand-written grammar for the first-order-form (FOF) fragment of
the TPTP language~\cite{Sut17}. The CLI uses \texttt{clap}~\cite{clap2024}
for argument parsing, \texttt{rusqlite}~\cite{rusqlite2024} for persisting
per-problem results to a SQLite database, and
\texttt{ctrlc}~\cite{ctrlc2024} for graceful cancellation under
\texttt{Ctrl+C}. The three search engines (\texttt{naive}, \texttt{id},
\texttt{priority-id}) are selected at the command line and share all code
outside the rule scheduler. The end-to-end pipeline is depicted in
Figure~\ref{fig:sysarch} (Sect.~\ref{sec:sysarch}).
