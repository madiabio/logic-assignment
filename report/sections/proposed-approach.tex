...
\texttt{backwards\_search} kernel unchanged.
...
$k{+}1$ is explored---and in particular before any deep
quantifier-instantiation chain is committed to, directly mitigating (F2).
...
engines support ablation: \texttt{id}, which applies iterative deepening
with the naive scheduler, and \texttt{naive}, the unmodified baseline.
...
We expect (F2) to be the dominant failure mode on provable benchmarks: the
naive DFS frequently descends into deep quantifier-instantiation chains and
times out before reaching a shallow proof that exists.  Iterative deepening
corrects this directly---every proof of depth $k$ is found before any branch
of depth $k{+}1$ is explored---so the \texttt{id} engine alone should account
for the majority of the additional solves over \texttt{naive}.
...
Delaying eigenvariable introduction until after propositional branching reduces
this compounding effect.  The visible-first/fresh-fallback split of
classes~5--6 provides a further tightening by keeping the term universe as
...
and the priority schedule providing a consistent additional gain concentrated
on problems with many quantified atoms.
